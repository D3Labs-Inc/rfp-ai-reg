# Title
Export of Consumable Stores Supplied to Vessels and Aircraft Permit


# ID
C.R.C.,_c._609

# URL
http://laws-lois.justice.gc.ca/eng/regulations/C.R.C.,_c._609/FullText.html


# Summary
EXPORT AND IMPORT PERMITS ACT General Export Permit No. Ex.
3. Short Title.
This Permit may be cited as the  Export of Consumable Stores Supplied to Vessels and Aircraft Permit .
Interpretation.
In this Permit,  consumable stores  includes fuel oils, lubricants, provisions and supplies required for a voyage or flight and medical and surgical supplies.
General.
Subject to section 15 of the  Export and Import Permits Act , any person may supply from Canada to vessels and aircraft departing from Canada usual and reasonable quantities of consumable stores where such stores are intended for use by or on the vessels and aircraft taking them on board.
Where completion and validation of a customs entry form is required with respect to any consumable stores that are exported under the authority of this Permit, that form shall be endorsed "Exported under the authority of General Export Permit No. Ex.
3" or "Exporté en vertu de la Licence générale d'exportation n o  Ex. 3".


# Tags
['permit', 'supply', 'general']


# Overlapping US Regulations
File Name: CFR-2018-title7-vol8, Reg (Part) Id: Pt. 980, URL:https://www.govinfo.gov/content/pkg/CFR-2018-title7-vol8/xml/CFR-2018-title7-vol8.xml

File Name: CFR-2018-title32-vol3, Reg (Part) Id: Pt. 525, URL:https://www.govinfo.gov/content/pkg/CFR-2018-title32-vol3/xml/CFR-2018-title32-vol3.xml

File Name: CFR-2018-title7-vol5, Reg (Part) Id: Pt. 355, URL:https://www.govinfo.gov/content/pkg/CFR-2018-title7-vol5/xml/CFR-2018-title7-vol5.xml

File Name: CFR-2018-title7-vol2, Reg (Part) Id: Pt. 35, URL:https://www.govinfo.gov/content/pkg/CFR-2018-title7-vol2/xml/CFR-2018-title7-vol2.xml

File Name: CFR-2018-title19-vol1, Reg (Part) Id: Pt. 122, URL:https://www.govinfo.gov/content/pkg/CFR-2018-title19-vol1/xml/CFR-2018-title19-vol1.xml




# NAICS Code
## Level 2 Codes
Code: 453, Title:Miscellaneous store retailers

Code: 452, Title:General merchandise stores

Code: 446, Title:Health and personal care stores




## Level 3 Codes
Code: 4461, Title:Health and personal care stores

Code: 5222, Title:Non-depository credit intermediation

Code: 3117, Title:Seafood product preparation and packaging







# Structured Analysis Summary
| Type      | Values                                            |
|:----------|:--------------------------------------------------|
| Condition | ['where']                                         |
| Entities  | ['Subject', 'Canada', 'Ex', 'Licence', 'Vessels'] |


# Structured Analysis With Context
 


## Condition
| Condition   | Context                                                                                                  |
|:------------|:---------------------------------------------------------------------------------------------------------|
| where       | Canada usual and reasonable quantities of consumable stores where such stores are intended for use by or |


## Entities
| Entities   | Context                                                                                 |
|:-----------|:----------------------------------------------------------------------------------------|
| Ex         | Ex .                                                                                    |
| Vessels    | as the Export of Consumable Stores Supplied to Vessels  and Aircraft Permit .           |
| Subject    | Subject to section 15 of the Export and Import                                          |
| Canada     | Permits Act , any person may supply from Canada  to vessels and aircraft departing from |
| Canada     | Permits Act , any person may supply from Canada  to vessels and aircraft departing from |
| Ex         | Ex .                                                                                    |
| Licence    | 3" or "Exporté en vertu de la  Licence  générale d'exportation n o  Ex.                 |


